...
namespace LCU.State.API.NapkinIDE.NapkinIDE.FathymForecast.Host
{
    public static class ConnectToState
    {
        ...
        public static async Task<ConnectToStateResponse> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post")]HttpRequest req, ILogger log,
            ...
            [SignalR(HubName = FathymForecastState.HUB_NAME)]IAsyncCollector<SignalRGroupAction> signalRGroupActions,
            [Blob("state-api/{headers.lcu-ent-api-key}/{headers.lcu-hub-name}/{headers.x-ms-client-principal-id}/{headers.lcu-state-key}", FileAccess.ReadWrite)] CloudBlockBlob stateBlob)
        {
            return await signalRMessages.ConnectToState<FathymForecastState>(req, log, claimsPrincipal, stateBlob, signalRGroupActions);
        }
    }
}
